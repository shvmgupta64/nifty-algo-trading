...
class BacktestEngine:
    ...
    def simulate_trade(self, df, entry_index, entry, sl, target, direction):
        """
        Enhanced intraday simulation with BREAK-EVEN SL:
        - When 1:1 RR is achieved, move SL to entry (cost)
        """
        ...
        # ---- 1:1 LEVEL ----
        if direction == "CE":
            break_even_level = entry + initial_risk
        else:
            break_even_level = entry - initial_risk

        sl_moved_to_cost = False  # flag for BE SL
        ...
        for j in range(entry_index + 1, len(df)):
            ...
            # ======================================================
            # 1️⃣ FIRST CHECK — Has Break-Even Triggered?
            # ======================================================
            if not sl_moved_to_cost:
                if direction == "CE" and high >= break_even_level:
                    sl = entry  # move SL to cost
                    sl_moved_to_cost = True

                elif direction == "PE" and low <= break_even_level:
                    sl = entry
                    sl_moved_to_cost = True

            # ======================================================
            # 2️⃣ After SL moved to cost → check BE SL hit
            # ======================================================
            if sl_moved_to_cost:
                if direction == "CE":
                    # Break-even SL hit
                    if low <= entry:
                        return "BE_HIT", entry, time

                    # Target hit
                    if high >= target:
                        return "TARGET_HIT", target, time
            ...
            # ======================================================
            # 3️⃣ Normal target / SL before BE move
            # ======================================================
            if direction == "CE":
                if high >= target:
                    return "TARGET_HIT", target, time
                if low <= sl:
                    return "SL_HIT", sl, time
            else:
                if low <= target:
                    return "TARGET_HIT", target, time
                if high >= sl:
